...
// we need to separate stuff that is going to be dumb and stuff that 
//is smart ,that is going to reference a previous value or get a value 

//the smart stuff or that which is going to get modified 
...
// returns an object that includes a function called render 
    return({
        name: 'circles',
        state,
        render:() => {
...
const simpleLines = (state) => {
    ...
    return({
        ...
        state,
        render :() => {
//return an object and render is one attribute inside that object
//that we would like to return
            ...
            for(let i=0; i<state.numShapes; i++){
                line(state.start*state.step,0,state.stop*state.stop,0);
                rotate(state.angle);
            }
            pop();
        }
